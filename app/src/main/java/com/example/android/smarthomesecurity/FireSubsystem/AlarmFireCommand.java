...
public class AlarmFireCommand extends AsyncTask<String,Void,String> {
    private String alarm;

    public AlarmFireCommand(String alarm) {
        this.alarm = alarm;
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            // Make a JSON payload with a state parameter for fire alarm and a name for the command which is command type called alarm
            JSONObject root2 = new JSONObject(json2);
            auth = root2.getString("access_token");
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("stat_alarm",alarm);
            ...
            // Check if command is not executed by fire alarm in wait timeout, it will send another command
            if(response.length()==0){
                Log.i("response","0");
                JSONObject root3 = new JSONObject(json2);
                auth = root3.getString("access_token");
                JSONObject jsonObject2 = new JSONObject();
                jsonObject2.put("stat_alarm",alarm);
                url = new URL(strings[0]);
                urlConnection = (HttpsURLConnection) url.openConnection();
                urlConnection.setRequestMethod("POST");
                urlConnection.setRequestProperty("Content-Type", "application/json");
