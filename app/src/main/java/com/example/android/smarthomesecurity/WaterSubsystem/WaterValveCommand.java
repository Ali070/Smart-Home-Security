...
public class WaterValveCommand extends AsyncTask<String,Void,String> {
    private String valve;

    public WaterValveCommand(String valve) {
        this.valve = valve;
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            // Make a JSON payload with a state parameter for water valve motors and a name for the command which is command type called watervalve
            JSONObject root2 = new JSONObject(json2);
            auth = root2.getString("access_token");
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("valve",valve);
            ...
            // Check if command is not executed by water valve motors in wait timeout, it will send another command
            if(response.length()==0){
                JSONObject jsonObject2 = new JSONObject();
                jsonObject2.put("valve",valve);
                url = new URL(strings[0]);
                urlConnection = (HttpsURLConnection) url.openConnection();
                urlConnection.setRequestMethod("POST");
                urlConnection.setRequestProperty("Content-Type", "application/json");
