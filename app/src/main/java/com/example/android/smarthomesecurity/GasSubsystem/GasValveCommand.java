...
public class GasValveCommand extends AsyncTask<String,Void,String> {
    private String gasValve;

    public GasValveCommand(String gasValve) {
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            // Make a JSON payload with a state parameter for gas valve motors and a name for the command which is command type called gas_open or gas_close
            JSONObject root2 = new JSONObject(json2);
            auth = root2.getString("access_token");
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("servo",gasValve);
            ...
            // Check if command is not executed by gas valve motors in wait timeout, it will send another command
            if(response.length()==0){
                JSONObject jsonObject2 = new JSONObject();
                jsonObject2.put("servo",gasValve);
                url = new URL(strings[0]);
                urlConnection = (HttpsURLConnection) url.openConnection();
                urlConnection.setRequestMethod("POST");
                urlConnection.setRequestProperty("Content-Type", "application/json");
