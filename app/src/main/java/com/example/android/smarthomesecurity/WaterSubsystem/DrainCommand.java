...
public class DrainCommand extends AsyncTask<String,Void,String> {
    private String drain;

    public DrainCommand(String drain) {
        this.drain = drain;
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            // Make a JSON payload with a state parameter for drain motors and a name for the command which is command type called drain
            JSONObject root2 = new JSONObject(json2);
            auth = root2.getString("access_token");
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("drain",drain);
            ...
            // Check if command is not executed by drain motors in wait timeout, it will send another command
            if(response.length()==0){
                JSONObject jsonObject2 = new JSONObject();
                jsonObject2.put("drain",drain);
                url = new URL(strings[0]);
                urlConnection = (HttpsURLConnection) url.openConnection();
                urlConnection.setRequestMethod("POST");
                urlConnection.setRequestProperty("Content-Type", "application/json");
